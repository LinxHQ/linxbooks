...
function setup()
{
    ...
    //connect database
   $connect = mysql_connect($mysql_host, $mysql_username, $mysql_password) or die('Error connecting to MySQL server: ' . mysql_error());
...
function addInformation($mysql_host,$mysql_username,$mysql_password,$mysql_database,$account_email,$account_pass)
{
    ...
    if(mysql_query($sql))
    {
         ...
         $id = $row['account_id'];
         $sql = "INSERT INTO lb_sys_account_profiles(account_id) VALUES (".$id.")";
         mysql_query($sql);
         
         // add subcription
         $sql1 = "INSERT INTO lb_sys_account_subscriptions(account_id,account_subscription_package_id,account_subscription_start_date,account_subscription_status_id) VALUES (".$id.",0,'".Date("Y-m-d H:i:s")."',1)";
         mysql_query($sql1);   
    }
}
