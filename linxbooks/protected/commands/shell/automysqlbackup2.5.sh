...
then
	...
	#then
		...
		# Prepare file name for next script to do the email sending task
                echo "$BACKUPFILES" > "$BACKUPDIR/last_file.txt"
	#else
	#	cat "$LOGFILE" | mail -s "WARNING! - MySQL Backup ($ATTSIZE) exceeds set maximum attachment size ($MAXATTSIZE) on $HOST - $DATE" $MAILADDR
	#fi
