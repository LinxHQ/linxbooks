...
class ht_twitter extends WP_Widget {
	...
	public function __construct() {
		global  $theme_name;
		$widget_ops = array('classname' => 'ht_twitter',
		...
		public function widget($args, $instance) {
			...
			if ( ! empty( $title ) ){ echo $before_title . $title . $after_title; }
				...
				if(!empty($tp_twitter_plugin_tweets)){
					print '
					<div class="recent-tweets" id="tweets_'.$args['widget_id'].'">
						<ul>';
						$fctr = '1';
						foreach($tp_twitter_plugin_tweets as $tweet){								
							print '<li>
							<i class="fa-twitter"></i>
							<span>'.tp_convert_links($tweet['text']).'</span>
							<br />
							<a class="twitterTime" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.tp_relative_time($tweet['created_at']).'</a></li>';
	...
	//widget settings form	
		public function form($instance) {
			...
			echo '
			<p><label>'.__('Title','highthemes').':</label>
			...
			<p><label>'.__('Exclude replies','highthemes').':</label>
