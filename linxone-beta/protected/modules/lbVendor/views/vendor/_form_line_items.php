...
$m = $this->module->id;
//LbVendorTotal::model()->totalAfterDiscount(49,LbVendorDiscount::LB_VENDOR_INVOICE_ITEM_TYPE_DISCOUNT,LbVendorTotal::LB_VENDOR_ITEM_TYPE_TOTAL);
						...
						'value' => '
							CHtml::activeTextField($data,"lb_vendor_item_quantity",
								array("style"=>"width: 50px;text-align: right; padding-right: 0px;
										...
										"data_invoice_id"=>"{$data->lb_vendor_id}",
										"line_item_pk"=>"{$data->lb_record_primary_key}",
                                                                                "onkeyup"=>"changeAmount({$data->lb_vendor_id},$data->lb_record_primary_key)",
...
echo CHtml::link(Yii::t('lang','Add item'), '#', array(
	'onclick'=>'
			$.post("'.$model->getActionURLNormalized('InsertBlankItem', array('id'=>$model->lb_record_primary_key)).'", 
                        function(response){
...
//{
    echo CHtml::ajaxSubmitButton(Yii::t('lang','Save'),
                    $model->getActionURLNormalized('ajaxUpdateLineItems',array('id'=>$model->lb_record_primary_key,'type'=>LbVendorItem::LB_VENDOR_ITEM_TYPE_LINE)), 
                    array(
...
echo CHtml::link(Yii::t('lang','Add discount'), '#', array(
    'onclick'=>'
        ...
        array('id'=>$model->lb_record_primary_key,'type'=>  LbVendorDiscount::LB_VENDOR_ITEM_TYPE_DISCOUNT)).'", function(response){
				var responseJSON = jQuery.parseJSON(response);
				if (responseJSON != null && responseJSON.success == 1)
				{
...
$this->widget('bootstrap.widgets.TbGridView', array(
    'id' => $discount_grid_id,
    'dataProvider' => $modelDiscountVendor->getVendorDiscounts($model->lb_record_primary_key,  LbVendorDiscount::LB_VENDOR_ITEM_TYPE_DISCOUNT),
    'hideHeader'=>true,
    ...
    'columns' => array(
        array(
            ...
            'deleteButtonUrl'=>'"' . $model->getActionURLNormalized("ajaxDeleteItemDiscount") . '" .
                                        "?id={$data->lb_record_primary_key}"',
            'afterDelete'=>'function(link,success,data){
                if(success) {
                    refreshTotals();
...
echo CHtml::ajaxSubmitButton(Yii::t('lang','Save Discounts'),
    $model->getActionURLNormalized('ajaxUpdateDiscounts',array('id'=>$model->lb_record_primary_key,'type'=>  LbVendorDiscount::LB_VENDOR_ITEM_TYPE_DISCOUNT)),
    array(
...
$this->widget('bootstrap.widgets.TbGridView', array(
    'id' => $taxes_grid_id,
    'dataProvider' => $modelTax->getTaxByVendor($model->lb_record_primary_key,  LbVendorTax::LB_VENDOR_ITEM_TYPE_TAX),
    'hideHeader'=>true,
    ...
    'columns' => array(
        array(
            'class'=>'bootstrap.widgets.TbButtonColumn',
            'template'=>"{delete}",
            'deleteButtonUrl'=>'"' . $model->getActionURLNormalized("ajaxDeleteItemTaxs") . '" .
                                        "?id={$data->lb_record_primary_key}"',
            'afterDelete'=>'function(link,success,data){
                if(success) {
                    refreshTotals();
...
echo CHtml::ajaxSubmitButton('Save Taxes',
 LbVendor::model()->getActionURLNormalized('ajaxUpdateTaxes',array('id'=>$model->lb_record_primary_key,'type'=>  LbVendorTax::LB_VENDOR_ITEM_TYPE_TAX)),
    array(
...
if($canEdit)
{
    ...
    if ($model->lb_vendor_status == $model::LB_PO_STATUS_CODE_DRAFT)
    {
        ...
        LBApplicationUI::ajaxButton(Yii::t('lang','Confirm Invoice'),
            $model->getActionURLNormalized('AjaxConfirmVenDor',
...
function onChangeTaxDropdown(line_item_pk, tax_id)
{
   ...
   var lb_tax_value = "";
    if (tax_id == -1)
    {
        ...
        $.post("<?php echo $model->getActionURLNormalized('ajaxGetTax',array('id'=>$model->lb_record_primary_key,'type'=>'vendor')); ?>",
...
{
    $.post("<?php echo $model->getActionURLNormalized('createBlankTax', array('id'=>$model->lb_record_primary_key,'type'=>  LbVendorTax::LB_VENDOR_ITEM_TYPE_TAX));?>",
        function(response){
            var responseJSON = jQuery.parseJSON(response);
            if (responseJSON != null && responseJSON.success == 1)
            {
...
function changeAmount(lb_vendor_id,lb_item_id)
{
    var quty = $('#lb_vendor_item_quantity'+lb_item_id).val();
    var prince = $('#lb_vendor_item_price'+lb_item_id).val();
    
//     $.post("<?php // echo $model->getActionURLNormalized('ajaxGetTax',array('id'=>$model->lb_record_primary_key)); ?>",
//            {lb_vendor_id: lb_vendor_id, lb_item_id: lb_item_id,},
//            function(response)
//            {
//                if (response != null)
//                {
//                    var responseJSON = jQuery.parseJSON(response);
                    ...
                    $("#lb_vendor_item_amount"+lb_item_id).val(quty*prince);
