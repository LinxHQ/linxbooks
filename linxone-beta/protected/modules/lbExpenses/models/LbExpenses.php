...
//		$criteria->compare('lb_expenses_no',$this->lb_expenses_no,true);
		$criteria->compare('lb_expenses_amount',$this->lb_expenses_amount,false);
...
public function convertNumberToWords($number) {
        ...
        if ($number < 0) {
        return $negative . LbExpenses::model()->convertNumberToWords(abs($number));
        }
        ...
        if ($remainder) {
        $string .= $conjunction . LbExpenses::model()->convertNumberToWords($remainder);
        }
        break;
        default:
        ...
        $string = LbExpenses::model()->convertNumberToWords($numBaseUnits) . ' ' . $dictionary[$baseUnit];
        if ($remainder) {
        $string .= $remainder < 100 ? $conjunction : $separator;
        $string .= LbExpenses::model()->convertNumberToWords($remainder);
        }
        break;
        }
        ...
        function convertNumberToWordsEng($number) {
    ...
    if ($number < 0) {
        return $negative . LbExpenses::model()->convertNumberToWordsEng(abs($number));
    }
    ...
    switch (true) {
        ...
        case $number < 1000:
            ...
            $string = $dictionary[$hundreds] . ' ' . $dictionary[100];
            if ($remainder) {
                $string .= $conjunction . LbExpenses::model()->convertNumberToWordsEng($remainder);
            }
            break;
        default:
            $baseUnit = pow(1000, floor(log($number, 1000)));
            $numBaseUnits = (int) ($number / $baseUnit);
            $remainder = $number % $baseUnit;
            $string = LbExpenses::model()->convertNumberToWordsEng($numBaseUnits) . ' ' . $dictionary[$baseUnit];
            if ($remainder) {
                $string .= $remainder < 100 ? $conjunction : $separator;
                $string .= LbExpenses::model()->convertNumberToWordsEng($remainder);
            }
            break;
    }
