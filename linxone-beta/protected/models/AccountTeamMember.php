...
//			return false;
	...
	public function getDropTeam($subscription_id, $get_all_records = FALSE)
	{
		$data = $this -> getTeamMembers($subscription_id, $get_all_records);
		$resuft = array();
		foreach ($data as $value) {
			$resuft[$value->member_account_id] = AccountProfile::model() -> getFullName($value->member_account_id);
		}
		return $resuft;
	}
...
//		$master_accounts_ids_str = implode(',', $master_accounts_ids);
	...
	public function getMasterAccountIDs($member_account_id, $project_id = 0)
	{
        ...
        /**
	 * Check if a team member to a master account is only a account admin
	 * @param integer $master_account_id
	 * @param integer $member_account_id
	 */
	public function isAcountAdmin($master_account_id=false, $member_account_id=false)
	{
            if(!$master_account_id)
                $master_account_id = AccountSubscription::model()->getSubscriptionOwnerID(Utilities::getCurrentlySelectedSubscription());
            if(!$member_account_id)
                $member_account_id = YII::app ()->user->id;
            
		$teamMember = AccountTeamMember::model()->find('master_account_id = :master_account_id AND member_account_id = :member_account_id',
				array(':master_account_id' => $master_account_id, ':member_account_id' => $member_account_id));
		
		if ($teamMember && $teamMember->is_account_admin == ACCOUNT_TEAM_MEMBER_IS_ACCOUNT_ADMIN)
			return true;
		return false;
	}
}
