...
//            return false;
	...
	public function actionAjaxUpdateField()
	{
        ...
        public function actionConfigsocial(){
            $value_action_social = $_REQUEST['value'];
            $social=LbConfigLoginSocial::model()->findByPk(1);
            $social->action=$value_action_social;
            $social->update();
        }
        public function actionConfigIdSocial(){
            $api_id = $_REQUEST['api_id'];
            $api_secret = $_REQUEST['api_secret'];
            $server_social = $_REQUEST['server_social'];
            if($server_social == "facebook"){
                $url_facebook = dirname(__FILE__).'/../config/config_id_facebook.php';
                $api_array = array('api_id_facebook' => $api_id, 'api_secret_facebook' => $api_secret);
                file_put_contents($url_facebook, '<?php $social_api_facebook = ' . var_export($api_array, true) . ';');
            } else if ($server_social == "google"){
                $url_google = dirname(__FILE__).'/../config/config_id_google.php';
                $api_array = array('api_id_google' => $api_id, 'api_secret_google' => $api_secret);
                file_put_contents($url_google, '<?php $social_api_google = ' . var_export($api_array, true) . ';');
            }
        }
        public function actionActivateUser(){
            $users_id_arr = $_REQUEST['user_id'];
            for($i=0;$i<count($users_id_arr);$i++) {
//                $users_id_arr[$i];
                // activate users
                $users=Account::model()->findByPk($users_id_arr[$i]);
                $users->check_user_activated=1;
                $users->update(); // save the change to databases
            }
        }
}
