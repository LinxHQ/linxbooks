...
class AccountInvitationController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$model_invite=new AccountInvitation;
		...
		{
			// tao 1 account, user name = $email, password = rand
			$rd_password = substr(str_shuffle(str_repeat("0123456789abcdefghijklmnopqrstuvwxyz", 5)), 0, 10);
			// end tao 1 account, user name = $email, password = rand
			$model=new Account;
			$exists_account = Account::model()->findAll('account_email IN ("'.$_POST['AccountInvitation']['account_invitation_to_email'].'")');
			if(count($exists_account) > 0) {
				$account_id = $exists_account[0]['account_id'];
				$account_subscription_id = $_POST['AccountInvitation']['account_invitation_subscription_id'];
				
				$criteria = new CDbCriteria();
				$criteria->condition = "member_account_id = $account_id";
				$criteria->condition = "account_subscription_id = $account_subscription_id";
				$account_team_member = AccountTeamMember::model()->findAll($criteria);
				$result = 0;
				// kiem tra ko cho phep add user vao mot team 2 lan.
				if(count($account_team_member) == 0){
					// create team member record
					$teamMember = new AccountTeamMember();
					$teamMember->accepting_invitation = true;
					$teamMember->master_account_id = 0;
					$teamMember->member_account_id = $exists_account[0]['account_id'];
		            $teamMember->account_subscription_id = $_POST['AccountInvitation']['account_invitation_subscription_id'];
					$teamMember->is_active = AccountTeamMember::ACCOUNT_TEAM_MEMBER_IS_ACTIVE;
					$teamMember->is_customer = $_POST['AccountInvitation']['account_invitation_type'];
					$result = $teamMember->save();

					if(isset($_POST['roles'])){
						foreach ($_POST['roles'] as $key => $value) {
							// neu tao thanh cong user thi gan quyen cho user do
							$roleAccount = new AccountRoles();
			                $roleAccount->accout_id = $exists_account[0]['account_id'];
			                $roleAccount->role_id = $value;
			                $roleAccount->save();
							// End neu tao thanh cong user thi gan quyen cho user do
						}
					}
				}

				if($result != 0){
					$account_profile = AccountProfile::model()->findAll('account_id IN ("'.$exists_account[0]['account_id'].'")');

					// gui thong tin dang nhap cho user bang email
					Yii::import('ext.yii-mail.YiiMailMessage');
					$message = new YiiMailMessage();
					$body_content = '<p>Hello '.$account_profile[0]['account_profile_given_name'].'</p>';
					$body_content .= "<p>You've just been assigned to a new team:. If you need further assistance, please contact the system administrator.</p>";
					$body_content .= '<p>Have a nice day!</p>';

					$message->setBody($body_content,'text/html');
					
					$message->setSubject('Account creation completed!');
					$message->addTo($_POST['AccountInvitation']['account_invitation_to_email']);
					$message->setFrom(Yii::app()->params['adminEmail']);
					$send_mail = Yii::app()->mail->send($message);
					// end gui thong tin dang nhap cho user bang email
				} else {
				...
				$model->account_email = $_POST['AccountInvitation']['account_invitation_to_email'];
				$model->account_password = $rd_password;
				$model->account_created_date = date('Y-m-d H:i');
				$model->account_status = ACCOUNT_STATUS_ACTIVATED;// ACCOUNT_STATUS_NOT_ACTIVATED;
				$model->account_language = $_POST['lbLangUser']['lb_language_name'];
				$model->save();

				// save account profile
				$model_profile=new AccountProfile;
				$model_profile->account_id = $model->account_id;
				$model_profile->account_profile_given_name = $_POST['AccountProfile']['account_profile_given_name'];
				$model_profile->save();
				// end save account profile
				...
				if(isset($_POST['roles'])){
					foreach ($_POST['roles'] as $key => $value) {
						// neu tao thanh cong user thi gan quyen cho user do
						$roleAccount = new AccountRoles();
		                $roleAccount->accout_id = $model->account_id;
		                $roleAccount->role_id = $value;
		                $roleAccount->save();
						// End neu tao thanh cong user thi gan quyen cho user do
					}
					...
					$teamMember->accepting_invitation = true;
					$teamMember->master_account_id = 0;
					$teamMember->member_account_id = $model->account_id;
		            $teamMember->account_subscription_id = $_POST['AccountInvitation']['account_invitation_subscription_id'];
					$teamMember->is_active = AccountTeamMember::ACCOUNT_TEAM_MEMBER_IS_ACTIVE;
					$teamMember->is_customer = $_POST['AccountInvitation']['account_invitation_type'];
					$teamMember->save();
				...
				// AccountInvitation[account_invitation_message]
				// gui thong tin dang nhap cho user bang email
				Yii::import('ext.yii-mail.YiiMailMessage');
				$message = new YiiMailMessage();
				$body_content = '<p>Dear '.$_POST['AccountInvitation']['account_invitation_message'].'</p>';
				$body_content .= '<p>An account has been successfully created, with the following username and password:</p>';
				$body_content .= '<p>Username: '.$_POST['AccountInvitation']['account_invitation_to_email'].'</p>';
				$body_content .= '<p>Password: '.$rd_password.'</p>';
				$body_content .= '<p>Have a nice day!</p>';
				$body_content .= '<p>System Admin</p>';
